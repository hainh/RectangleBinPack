...
struct node
{
	...
	void delcheck()
	{
		if (c[0].pn)
		{
			c[0].fill = false; c[0].pn->delcheck();
		}
		if (c[1].pn)
		{
			c[1].fill = false; c[1].pn->delcheck();
		}
	}
...
rect_wh _rect2D(rect_xywhf* const * v, int n, int max_w, int max_h, vector<rect_xywhf*>& succ, vector<rect_xywhf*>& unsucc)
{
	node root;
	...
	rect_wh min_bin = rect_wh(max_w, max_h);
...
bool pack(rect_xywhf* const * v, int n, int max_w, int max_h, vector<bin>& bins)
{
	rect_wh _rect(max_w, max_h);
	...
	while (true)
	{
		...
		b->size = _rect2D(&((*p[0])[0]), p[0]->size(), max_w, max_h, b->rects, *p[1]);
...
/*
Find best bin by step increasing width and height of bin. The fisrt bins that fit all rects by increasing width and height
compare to each other will be the best fit bin.
*/
bool auto_pack(rect_xywhf* const * v, int n, std::vector<bin>& bins)
{
	// Find max width and height of all rects and step to increase width and height
	int area_total = 0;
	int step = 1000000, max_w = 0, max_h = 0;
	int w, h;
	for (int i = 0; i < n; ++i)
	{
		area_total += v[i]->area();
		...
		step = w < step ? w : h < step ? h : step;
	}
	// step at least 10 to improve performance
	const int min_step = 30;
	step = step > min_step ? step : min_step;

	int start_wh = (int)(std::sqrtf(area_total) / 3);
	// max of width and height to test
	int max_wh = start_wh * 8;
	// start width for the increasing
	int start_w = start_wh > max_w ? start_wh : max_w;
	// and start height
	int start_h = start_wh > max_h ? start_wh : max_h;

	int min_area = std::numeric_limits<int>::max();

	// contain temp area variable
	int area_tm;

	for (w = start_w; w < max_wh; w += step)
	{
		int opt_start_h = start_h > area_total / w ? start_h : area_total / w;
		for (h = opt_start_h; h < max_wh; h += step)
		{
			std::vector<bin> bins_tm;
			if (pack(v, n, w, h, bins_tm) && bins_tm.size() == 1)
			{
				area_tm = bins_tm[0].size.area();
				if (area_tm < min_area)
				{
					min_area = area_tm;
					if (!bins.empty())
					{
						auto rects = bins[0].rects;
						for (auto rect : rects)
						{
							delete rect;
						}
					...
					bins.push_back(bins_tm[0]);
					auto & rects_tm = bins[0].rects;
					for (unsigned i = 0; i < rects_tm.size(); ++i)
					{
						rects_tm[i] = new rect_xywhf(*rects_tm[i]);
					}
				}
				// first bin meet is smallest bin in this loop by h, so break when done
				break;
			}
		}
	}
